...
library Tick {
    int24 internal constant MAXIMUM_TICK_SPACING = 16_384;

    // info stored for each initialized individual tick
    struct Info {
        /// @dev the total per-tick liquidity that references this tick (either as tick lower or tick upper)
        uint128 liquidityGross;
        /// @dev amount of per-tick liquidity added (subtracted) when tick is crossed from left to right (right to
        /// left),
        int128 liquidityNet;
        PositionBalances growthOutsideX128;
        /// @dev true iff the tick is initialized, i.e. the value is exactly equivalent to the expression liquidityGross
        /// != 0
    ...
    /// @return liquidityNet The amount of liquidity added (subtracted) when tick is crossed from left to right (right
    /// to left)
    function cross(
