...
contract ScenarioLiq is ScenarioSetup, AssertionHelpers, Actions, Checks {
    ...
    function test_scenario_liquidations() public {
        setConfigs();
...

        {
            PositionInfo memory positionInfo =
                PositionInfo({ accountId: 1, marketId: marketId, maturityTimestamp: maturityTimestamp });
            /// @dev price limit does not matter here
            checkValidLiquidationOrder(datedIrsProxy, positionInfo, -100 * 1e6, 0, true);
            checkValidLiquidationOrder(datedIrsProxy, positionInfo, -1000 * 1e6, 0, false); // max base 9999
        }

        executeLiquidation({
            liquidatableAccountId: 2,
            liquidatorAccountId: 3,
            marketId: marketId,
            maturityTimestamp: maturityTimestamp,
            baseAmountToBeLiquidated: 500 * 1e6,
            priceLimit: 0.04 * 1e18
        });

        // check account 3 (liquidator)
        {
            PositionInfo memory positionInfo =
            ...
            checkFilledBalancesWithoutUPnL({
                datedIrsProxy: datedIrsProxy,
                positionInfo: positionInfo,
                expectedBaseBalance: -500000010,
                expectedQuoteBalance: 25252070,
                expectedRealizedPnL: 0
            });
        }

        // check account 2 (liquidator)
        {
            PositionInfo memory positionInfo =
            ...
            checkFilledBalancesWithoutUPnL({
                datedIrsProxy: datedIrsProxy,
                positionInfo: positionInfo,
                expectedBaseBalance: -499999990,
                expectedQuoteBalance: 23104506,
                expectedRealizedPnL: 0
            });
        }
