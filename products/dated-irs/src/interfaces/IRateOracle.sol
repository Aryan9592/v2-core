...
interface IRateOracle is IERC165 {
    ...
    /// matters is that we can divide one "liquidity index" by another to get the factor of growth between the two
    /// timestamps.
    /// For example if we have indices of { (t=0, index=5), (t=100, index=5.5) }, we can divide 5.5 by 5 to get a growth
    /// factor
    /// of 1.1, suggesting that 10% growth in capital was experienced between timesamp 0 and timestamp 100.
    /// @dev The liquidity index is normalised to a UD60x18 for storage, so that we can perform consistent math across
    /// all rates.
